...
// hw 5
char id[10][ CONSOLESHELL_MAXCOMMANDBUFFERCOUNT ];
char password[10][ CONSOLESHELL_MAXCOMMANDBUFFERCOUNT ];
char mod[10][ CONSOLESHELL_MAXCOMMANDBUFFERCOUNT ];
int userNum = 0;
int login = -1;
...
SHELLCOMMANDENTRY gs_vstCommandTable[] =
{
		...
		//hw추가구현
		{ "saveuser", "delete", kSaveUser },
        { "chmod", "Change file Authority", kChangeAuthority },
        { "chown", "Change file OwnerID, ex) chown temp a.txt", kChangeOwnerID },
		{ "createid", "Create ID/PASS/MOD", kCreateId},
		{ "changepassword", "change password!", kChangePass},		
};
...
void kStartConsoleShell( void )
{
    ...
    if(loginStep == 0){
		kPrintf( "Enter User ID & Password\n");
		kPrintf( CONSOLESHELL_ID );
    }
    else{
    	kPrintf( "\nSetting ROOT password first\n");
    	kPrintf( CONSOLESHELL_PASSWORD );
    }
	...
	while( 1 )
	{
		...
		else if( bKey == KEY_ENTER )
		{
			...
			if( iCommandBufferIndex > 0 )
			{
				vcCommandBuffer[ iCommandBufferIndex ] = KEY_ENTER;

				//id check
				if(loginStep==0){				
					for(int i=0;i<userNum;i++){
						if(kMemCmp( vcCommandBuffer,  id[i] , kStrLen(id[i])) == 0 ){
							login=i;
							loginStep++;
							break;
						}
				...
				// pass & check
					...
					for(int i=0;i<userNum;i++){
						if(kMemCmp( vcCommandBuffer,  password[i] , kStrLen(password[i])) == 0 ){						
							loginStep++;
							break;
						}
					}
				...
				// no data
					...
					int j =0;
					while(1){
						password[0][j]=vcCommandBuffer[j];
						if(password[0][j]==KEY_ENTER){
							kMountHDD( vcCommandBuffer );
							FILE* crt;
							BYTE b;
							crt = fopen( "user.txt", "w" );
							fclose( crt );
							kSaveUser();
							login = 0;
							break;
			...
			// id pass
			if (loginStep==1) kPrintf( "%s", CONSOLESHELL_PASSWORD );

			// fail
			else if (loginStep==0) {
				kPrintf( "You got wrong ID or Password.\n");
				kPrintf( "%s", CONSOLESHELL_ID );
			}
...
BOOL kIsRightFormat(const char* vAuthority)
{
  /*
  1. 숫자인지
      00 / 01 / 10 / 11 => 최대 3
      33 => 이런식으로 입력
        제한 / 두자리여야하고, 각자리수 3보다 크면 안됨
  */
  if(vAuthority[0] >= '0' && vAuthority[0] <= '3' && vAuthority[1] >= '0' && vAuthority[1] <= '3'&& vAuthority[2] == '\0')
  {
      return 1;
  }
  /*
  2. 심볼릭인지
  else if(enteredMode[0] == 'u' || enteredMode[0] == 'o')
  {
...
static void kCreateFileInRootDirectory( const char* pcParameterBuffer )
{
    ...
    getdir(pstFile->stFileHandle.iDirectoryEntryOffset, &stEntry);
    kMemCpy( stEntry.vcOwnerID, id[login], kStrLen( id[login] ) + 1 );
    setdir(pstFile->stFileHandle.iDirectoryEntryOffset, &stEntry);
    
	id[login][t]=KEY_ENTER;
...
static void kDeleteFileInRootDirectory( const char* pcParameterBuffer )
{
    ...
    //hw추가구현 읽기권한확인 ( 지금 안되는 부분 : userID 받아오기)
    ...
    getdir(fp->stFileHandle.iDirectoryEntryOffset, &stEntry);
    //ID 루트이면 통과
    if( mod[login][0]=='r' )
    {
    }
    //ID 소유자이면 확인
    else if( (kMemCmp( id[login],  stEntry.vcOwnerID , kStrLen(stEntry.vcOwnerID))) == 0 ){
      //소유자권한 w이면 통과
      if( stEntry.vcAuthority[1] == 'w' )
      {
      }
      else
      {
        kPrintf( "No Authority\n" );
        return ;
      }
    }
    else
...
static void kShowRootDirectory( const char* pcParameterBuffer )
{
    ...
    while( 1 )
    {
        ...
        kMemCpy( vcBuffer+10, pstEntry->vcOwnerID,
                 kStrLen( pstEntry->vcOwnerID ) );

        kMemCpy( vcBuffer+20, pstEntry->vcAuthority,
                 kStrLen( pstEntry->vcAuthority ) );
...
static void kWriteDataToFile( const char* pcParameterBuffer )
{
    ...
    //hw추가구현 쓰기권한확인 ( 지금 안되는 부분 : userID 받아오기)
    getdir(fp->stFileHandle.iDirectoryEntryOffset, &stEntry);
    //ID 루트이면 통과
    if( mod[login][0]=='r' )
    {
    ...
    {        
        ...
        else if( bKey == KEY_ENTER )
        {
            ...
            if( iCommandBufferIndex > 0 )
            {
            	...
            	if( step == 0){            		
            		while(1){
            			id[userNum][j]=vcCommandBuffer[j];
            			
            			if(id[userNum][j]==KEY_ENTER){
            				step++;
            				kPrintf("Type want Password\n");
            				kPrintf( "%s", CONSOLESHELL_PASSWORD);
            				break;
            			}
            			j++;
            		}
            		...
            		while(1){
                		password[userNum][j]=vcCommandBuffer[j];
                		if(password[userNum][j]==KEY_ENTER){
                			step=3;
                			if(mod[login][0]=='r'){
                				step--;
								kPrintf("Type Mode ('r' == root, 'o' == other)\n");
								kPrintf( "MODE>");
                			}
                			else{
                				mod[userNum][0]='o';
                				mod[userNum][1]=KEY_ENTER;
                				userNum++;
                			}
                			break;                			
                		}
            		...
            		while(1){
                		mod[userNum][j]=vcCommandBuffer[j];
                		if(password[userNum][j]==KEY_ENTER){
                			step++;
                			userNum++;
                			break;
...
static void kChangePass(void){
	...
	kPrintf("\ncurrent ID = %s\n",id[login]);
	kPrintf("Type current PASSWORD first\n");
	//kPrintf("Type want to change PASSWORD\n");
	kPrintf( "%s", CONSOLESHELL_PASSWORD);
    ...
    {        
        ...
        else if( bKey == KEY_ENTER )
        {
            ...
            if( iCommandBufferIndex > 0 )
            {
            	...
            	if( step == 0){            		
            		while(1){
            			if( password[login][j]!=vcCommandBuffer[j]){
            				break;
            			}            			
            			if(password[login][j]==KEY_ENTER){
            				step=1;
            				break;
            			}
            			j++;
            		...
            		while(1){
            			password[login][j]=vcCommandBuffer[j];            				            			            		
            			if(password[login][j]==KEY_ENTER){
            				step=3;
            				break;
            			}
            ...
            if(step == 0){
            	kPrintf("You type wrong password, please type again.\n");
            	kPrintf( "%s", CONSOLESHELL_PASSWORD);
            }            
            else if(step==1){
            	kPrintf("Type want to change PASSWORD\n");
...
static void kChangeAuthority( const char* pcParameterBuffer ){
  ...
  DIRECTORYENTRY stEntry;
  char enteredMode[ 50 ];
  char newAuthority[ 50 ];
  char vcFileName[ 50 ];
  int iLength;
  FILE* fp;

  kInitializeParameter( &stList, pcParameterBuffer );
  iLength = kGetNextParameter( &stList, enteredMode );
  enteredMode[ iLength ] = '\0';
  if( ( iLength >  9 ) || ( iLength == 0 ) )
  {
  ...
  if(kIsRightFormat( enteredMode ))
  {
    for (int i = 0; i < 2; i++)
    {
      ...
      else if(enteredMode[i] == '1')
      {
        newAuthority[(i * 2)] = '-';
        newAuthority[(i * 2 + 1)] = 'w';
      }
      else if(enteredMode[i] == '2')
      {
        newAuthority[(i * 2)] = 'r';
        newAuthority[(i * 2 + 1)] = '-';
      }
      else if(enteredMode[i] == '3')
      {
        newAuthority[(i * 2)] = 'r';
        newAuthority[(i * 2 + 1)] = 'w';
      }
  ...
  kMemCpy( stEntry.vcAuthority, newAuthority, kStrLen( newAuthority ) + 1 );
  setdir(fp->stFileHandle.iDirectoryEntryOffset, &stEntry);


  kPrintf("File Authority has Changed\n" );
  fclose( fp );

}

//hw추가구현 소유자변경(root계정만 가능)
//ex) chown root user.txt
static void kChangeOwnerID( const char* pcParameterBuffer ){
  ...
  {
    kInitializeParameter( &stList, pcParameterBuffer );
    iLength = kGetNextParameter( &stList, newOwnerID );
    newOwnerID[ iLength ] = '\0';
    if( ( iLength > ( FILESYSTEM_MAXFILENAMELENGTH - 1 ) ) || ( iLength == 0 ) )
    {
        kPrintf( "Too Long or Too Short ID\n" );
        return ;
    }
    ...
    while(1){
    	if (i>=userNum){
    		kPrintf( "Type Wrong ID, please do again\n" );
    		return;
    	}
    	if (kMemCmp( id[i],  newOwnerID , kStrLen(newOwnerID)) == 0){
    		break;
    	}
    	i++;
    }
    ...
    getdir(fp->stFileHandle.iDirectoryEntryOffset, &stEntry);
    kMemCpy( stEntry.vcOwnerID, newOwnerID, kStrLen( newOwnerID ) + 1 );
    setdir(fp->stFileHandle.iDirectoryEntryOffset, &stEntry);

    kPrintf("File Owner has Changed\n" );
